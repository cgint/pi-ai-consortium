#!/usr/bin/env python3
"""Fresh c04 supersession-guard runner with explicit end-to-end model identity."""
...
CORPUS_PATH = HERE / "c04-supersession-corpus.json"
CONTRACT_PATH = HERE / "c04-contract-files.json"
REVIEW_SESSION_PATH = REPO_ROOT / "docs" / "c04-evidence" / "independent-review-8081-twins-session.jsonl"
LEDGER_PATH = REPO_ROOT / "docs" / "c04-evidence" / "raw-publication-ledger.json"
STATE_FIXTURE_PATH = HERE / "c04-fixtures" / "pi-0.84.1-get-state.json"
STATE_PROVENANCE_PATH = HERE / "c04-fixtures" / "provenance.json"
PI_VERSION = "0.84.1"
...
def validate_executor_state(response: Dict[str, Any]) -> Dict[str, Any]:
    data = response.get("data") if isinstance(response, dict) else None
    data = data if isinstance(data, dict) else {}
    model = data.get("model")
    model = model if isinstance(model, dict) else {}
    observed = {"provider": model.get("provider"), "model": model.get("id"), "thinking": data.get("thinkingLevel")}
    checks = {
        "response_success": response.get("success") is True if isinstance(response, dict) else False,
        "provider": observed["provider"] == MODEL_PROVIDER,
        "model": observed["model"] == MODEL_ID,
        "thinking": observed["thinking"] == THINKING_LEVEL,
    }
    return {"pass": all(checks.values()), "checks": checks, "observed": observed}


def verify_state_fixture() -> Dict[str, Any]:
    if not STATE_FIXTURE_PATH.is_file() or not STATE_PROVENANCE_PATH.is_file():
        raise RuntimeError("c04 captured state fixture/provenance missing")
    provenance = json.loads(STATE_PROVENANCE_PATH.read_text())
    fixture_sha = sha256_file(STATE_FIXTURE_PATH)
    checks = {
        "provenance_schema": provenance.get("schema_version") == "c04-get-state-provenance-v1",
        "capture_runtime": provenance.get("capture_runtime") == "Pi 0.84.1",
        "byte_identity": provenance.get("source_sha256") == fixture_sha == provenance.get("fixture_sha256"),
        "use_boundary": "not c04 evidence" in str(provenance.get("use_boundary", "")),
        "nested_identity": validate_executor_state(json.loads(STATE_FIXTURE_PATH.read_text()))["pass"],
    }
    if not all(checks.values()):
        raise RuntimeError(f"c04 captured state fixture gate failed: {checks}")
    return {"checks": checks, "fixture_sha256": fixture_sha, "provenance": provenance}


def load_corpus() -> Dict[str, Dict[str, Any]]:
    data = json.loads(CORPUS_PATH.read_text())
    if data.get("schema_version") != "c04-supersession-corpus-v1":
        raise ValueError("unsupported c04 corpus schema")
    fixtures = data.get("fixtures")
    if not isinstance(fixtures, list) or len(fixtures) != 8:
        raise ValueError("c04 corpus must contain exactly eight fixtures")
    mapped = {item.get("id"): item for item in fixtures if isinstance(item, dict) and isinstance(item.get("id"), str)}
    if len(mapped) != 8:
        raise ValueError("c04 corpus fixture IDs must be unique")
    if sum(item.get("kind") == "positive" for item in mapped.values()) != 4 or sum(item.get("kind") == "control" for item in mapped.values()) != 4:
        raise ValueError("c04 corpus must contain four positive and four control fixtures")
    for item in mapped.values():
        target = item.get("target")
        if not isinstance(target, str) or Path(target).is_absolute() or ".." in Path(target).parts:
            raise ValueError("c04 fixture target is invalid")
        if not isinstance(item.get("before"), str) or not isinstance(item.get("prompts"), list) or len(item["prompts"]) != 3:
            raise ValueError("c04 fixture must provide before text and exactly three prompts")
    return mapped
...
RUN_SPECS = [
    {"run_id": f"c04-{arm}-r{repetition}-{fixture_id}", "arm": arm, "repetition": repetition, "fixture_id": fixture_id}
...
def arm_guard_enabled(spec: Dict[str, Any]) -> bool:
    ...
    if arm not in ("off", "on"):
        raise ValueError("c04 arm must be off or on")
    return arm == "on"
...
def verify_review_session(expected_sha256: str) -> Dict[str, Any]:
    if not REVIEW_SESSION_PATH.is_file() or sha256_file(REVIEW_SESSION_PATH) != expected_sha256:
        raise RuntimeError("c04 review session evidence mismatch")
    ...
    checks = {
        "session_name": isinstance(info, dict) and str(info.get("name", "")).startswith("c04-8081-twins"),
    ...
    if not all(checks.values()):
        raise RuntimeError(f"c04 review session gate failed: {checks}")
...
def verify_publication_ledger(require_all_unconsumed: bool) -> Dict[str, Any]:
    if not LEDGER_PATH.is_file():
        raise RuntimeError("c04 raw-publication ledger is missing")
    data = json.loads(LEDGER_PATH.read_text())
    if data.get("schema_version") != "c04-raw-publication-ledger-v1" or not isinstance(data.get("runs"), list):
        raise RuntimeError("c04 raw-publication ledger schema invalid")
    expected_ids = [spec["run_id"] for spec in RUN_SPECS]
    records = data["runs"]
    if [record.get("run_id") for record in records] != expected_ids:
        raise RuntimeError("c04 raw-publication ledger order/IDs mismatch")
    for record in records:
        run_id = record["run_id"]
        expected_dir = f"docs/c04-raw/{run_id}"
        if record.get("raw_directory") != expected_dir or not (REPO_ROOT / expected_dir).is_dir():
            raise RuntimeError(f"c04 raw destination mismatch: {run_id}")
        ...
        if tracked.returncode != 0:
            raise RuntimeError(f"c04 raw destination not tracked: {run_id}")
        if require_all_unconsumed and record.get("status") != "unconsumed":
            raise RuntimeError(f"c04 run is already consumed before prompt 1: {run_id}")
    return data


def verify_contract(expected_sha256: str, freeze_commit: str) -> Dict[str, Any]:
    if sha256_file(CONTRACT_PATH) != expected_sha256:
        raise RuntimeError("c04 contract SHA-256 mismatch")
    data = json.loads(CONTRACT_PATH.read_text())
    if data.get("schema_version") != "c04-contract-files-v1" or not isinstance(data.get("files"), list):
        raise RuntimeError("c04 contract schema invalid")
    ancestor = subprocess.run(["git", "merge-base", "--is-ancestor", freeze_commit, "HEAD"], cwd=REPO_ROOT, check=False)
    if ancestor.returncode != 0:
        raise RuntimeError("c04 freeze commit is not an ancestor of HEAD")
    paths: set[str] = set()
    for item in data["files"]:
        if not isinstance(item, dict) or set(item) != {"path", "sha256"}:
            raise RuntimeError("c04 contract entry invalid")
        rel, expected = item["path"], item["sha256"]
        if not isinstance(rel, str) or rel in paths or rel.startswith("/") or ".." in Path(rel).parts:
            raise RuntimeError("c04 contract path invalid")
        paths.add(rel)
        target = REPO_ROOT / rel
        if not target.is_file() or sha256_file(target) != expected:
            raise RuntimeError(f"c04 current contract file mismatch: {rel}")
        ...
        if frozen.returncode != 0 or p.sha256_bytes(frozen.stdout) != expected:
            raise RuntimeError(f"c04 frozen contract file mismatch: {rel}")
    if not paths:
        raise RuntimeError("c04 contract has no files")
    return data
...
class C04Runner(p.Phase05Runner):
    ...
    def _validate_frozen_inputs(self) -> None:
        if self.spec != RUN_BY_ID.get(self.run_id):
            raise RuntimeError("c04 run ID does not match frozen schedule")
        ...
        state_fixture = verify_state_fixture()
        ...
        if sha256_file(Path(__file__).resolve()) != self.expected_runner_sha:
            raise RuntimeError("c04 runner SHA mismatch")
        if sha256_file(CORPUS_PATH) != self.corpus_sha256:
            raise RuntimeError("c04 corpus SHA mismatch")
        ...
        self.frozen_checks = {
            ...
            "captured_state_schema": all(state_fixture["checks"].values()),
        ...
        if failed:
            raise RuntimeError(f"c04 frozen identity checks failed: {failed}")
    ...
    def _guard_existing_paths(self, all_targets: bool) -> None:
        ...
        if conflicts:
            raise FileExistsError(f"refusing existing c04 targets: {conflicts}")
    ...
    def _build_manifest(self) -> Dict[str, Any]:
        ...
        node = p.cmd_output(["node", "--version"])
        self.manifest = {
            "schema_version": "c04-run-manifest-v1", "run_id": self.run_id, "arm": self.spec["arm"],
    ...
    def preflight(self) -> Dict[str, Any]:
        try:
            ...
            return {"schema_version": "c04-preflight-v1", "run_id": self.run_id, "pass": True, "prompts_delivered": 0, "manifest": self.manifest}
        except Exception as exc:
            return {"schema_version": "c04-preflight-v1", "run_id": self.run_id, "pass": False, "prompts_delivered": 0, "exception": f"{type(exc).__name__}: {exc}", "manifest": self.manifest}
    ...
    def _materialize_workspace(self) -> None:
        ...
        if persisted is not enabled:
            raise RuntimeError("c04 workspace guard setting mismatch before Pi launch")
    ...
    def _run_rpc_loop(self, proc: subprocess.Popen, rpc_log_path: Path, stderr_path: Path) -> None:
        ...
        sequencer = C04Sequencer(self.fixture["prompts"])
        reported_errors = 0
    ...
    def _validate_all(self) -> List[p.Assertion]:
        ...
        add("C04-process", rc == 0, f"returncode={rc}")
        initial = validate_executor_state(self.responses.get("get_state", {}))
        final = validate_executor_state(self.responses.get("state_final", {}))
        add("C04-executor-provider", initial["checks"]["provider"] and final["checks"]["provider"], f"initial={initial['observed']['provider']}; final={final['observed']['provider']}")
        add("C04-executor-model", initial["checks"]["model"] and final["checks"]["model"], f"initial={initial['observed']['model']}; final={final['observed']['model']}")
        add("C04-executor-thinking", initial["checks"]["thinking"] and final["checks"]["thinking"], f"initial={initial['observed']['thinking']}; final={final['observed']['thinking']}")
        prompts = [item for item in self.outgoing_commands if item.get("type") == "prompt"]
        add("C04-prompts", [item.get("message") for item in prompts] == self.fixture["prompts"], f"count={len(prompts)}")
        add("C04-protocol", not self.protocol_errors, f"errors={self.protocol_errors}")
        identities = self.manifest.get("identity_checks", {})
        add("C04-preflight-identities", bool(identities) and all(value is True for value in identities.values()), f"checks={identities}")
        ...
        add("C04-trace-identity", trace_valid, f"starts={len(starts)}; models={sorted({str(event.get('model')) for event in starts})}; sources={sorted({str(event.get('modelSource')) for event in starts})}")
        fired = guard_fired(self.consortium_events)
        expected_guard = self.spec["arm"] == "on" and self.fixture["kind"] == "positive"
        add("C04-guard", fired == expected_guard, f"expected={expected_guard}; observed={fired}")
        target = self.workspace / self.fixture["target"]
        text = target.read_text() if target.exists() else ""
        continuity = continuity_passes(self.fixture, text)
        add("C04-continuity", continuity, f"positive={self.fixture['kind'] == 'positive'}; continuity={continuity}")
        return assertions
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="c04 fresh supersession guard runner")
    ...
    runner = C04Runner(RUN_BY_ID[args.run_id], args.freeze_commit, args.runner_sha256, args.corpus_sha256, args.contract_sha256, args.review_session_sha256)
    result = runner.preflight() if args.preflight_only else runner.run()
    print(json.dumps(result, indent=2, default=str))
    if result.get("pass") is True: return 0
    identity_ids = {"C04-process", "C04-executor-provider", "C04-executor-model", "C04-executor-thinking", "C04-prompts", "C04-protocol", "C04-preflight-identities", "C04-trace-identity"}
