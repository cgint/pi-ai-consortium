# c04 supersession-guard uplift preregistration

> **Status:** PROPOSED FREEZE — no c04 preflight, workspace, or prompt until the committed exact review gate passes.
> **Scope:** Fresh c04 IDs/evidence only. c03 remains permanently harness-invalid/no-uplift; c01/c02 remain unchanged historical evidence.
...
![c04 prospective flow](./c04-supersession-preregistration.svg)
...
- Executor state schema: Pi 0.84.1 provider/model are read only from `get_state.data.model.provider` and `.id`; thinking is read from `get_state.data.thinkingLevel`.
- Captured contract: `scripts/behavioral-parcour/c04-fixtures/pi-0.84.1-get-state.json` is a c04-owned byte-identical schema fixture with explicit provenance. Obsolete top-level provider/model fields must fail.
...
`scripts/behavioral-parcour/c04-supersession-corpus.json` owns the c04 copy of eight fixtures: four positive supersession fixtures and four matched controls, each with three fixed prompts. No c01/c02/c03 run artifact is a c04 gate or repetition. The captured c03 state response is used only as attributed schema-fixture source material.
...
1. Commit the c04 runner, tests, captured state fixture/provenance, corpus, contract, preregistration, diagram, 48 raw destinations, and unconsumed ledger.
2. Pass c04 unit tests—including captured nested-state success and obsolete/malformed-state failures—contract verification, full test/typecheck, and `npm run precommit`.
3. Complete a new RLM review under exactly `8081-twins/qwen36-27b-nvidia-nvfp4:off` after the c04 freeze; commit its raw output and Pi session JSONL.
4. Preflight verifies the captured state fixture/provenance, committed contract at the freeze commit, review identity/verdict/timing, Pi/Node, executor command, extension order, and effective child environment before materialization.
5. `--preflight-only` must leave every `/tmp/parcour-c04-*` and `.parcour-runs/c04-*` path absent.

A later review, rehash, source correction, provider substitution, or prior-run evidence cannot cure a consumed c04 cell.
